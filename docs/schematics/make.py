...
mat(ax, counts, cmap="Greys", vmin=0, vmax=3, xl="cells (all donors)", yl="genes")
ax2 = sub(fig, P[0], (0.08, 0.715, 0.84, 0.05))
dc = ["#bdbdbd", "#9e9e9e", "#d64545", "#bdbdbd", "#9e9e9e"]
...
bx = sub(fig, P[1], (0.06, 0.08, 0.86, 0.22)); bx.set_axis_off(); bx.set_xlim(0, 1); bx.set_ylim(0, 1)
bx.add_patch(FancyBboxPatch((0.02, 0.1), 0.52, 0.8, boxstyle="round,pad=0.02", fc="white", ec="#e08a2e", lw=1.4))
bx.text(0.28, 0.5, "fit SCTransform v2\n3000 HVGs  (must = saved)", ha="center", va="center", fontsize=8)
bx.add_patch(FancyArrowPatch((0.56, 0.5), (0.72, 0.5), arrowstyle="-|>", mutation_scale=12, color="#e08a2e", lw=1.4))
bx.text(0.87, 0.5, "apply same\nmodel", ha="center", va="center", fontsize=8)
f1 = FancyArrowPatch((0.25, 0.44), (0.25, 0.31), transform=fig.transFigure, arrowstyle="-|>", mutation_scale=10, color="#e08a2e")
x0, w = P[1]
...
# ---------------- Step 2 ----------------
fig, P = frame("Step 2  Correlations + reference evidence")
ref_lab = np.repeat([0, 1, 2, 3], [9, 8, 9, 8])
prof = rng.normal(0, 1, (4, 22))
...
txt(fig, P[0], 0.08, 0.07, "reference cell-type labels + donors", fontsize=8, color=MUTED)
# does
zi = (Q[:, 0] - Q[:, 0].mean()) / Q[:, 0].std()
ax = sub(fig, P[1], (0.08, 0.30, 0.09, 0.50)); mat(ax, zi[:, None], vmin=-2.5, vmax=2.5, title="z$_i$")
for k in range(4):
    zk = R[:, ref_lab == k].mean(1); zk = (zk - zk.mean()) / zk.std()
    ax = sub(fig, P[1], (0.40 + k * 0.13, 0.30, 0.09, 0.50)); mat(ax, zk[:, None], vmin=-2.5, vmax=2.5)
    ax.set_title(r"$\bar z_%s$" % TN[k], fontsize=9, color=TC[k])
txt(fig, P[1], 0.22, 0.53, "·", fontsize=22)
txt(fig, P[1], 0.40, 0.89, "mean z-profile per type", fontsize=8, color=MUTED)
txt(fig, P[1], 0.08, 0.15, r"$s_{ik} = z_i^\top \bar z_k/(G-1)$" + "  = mean Pearson r to type k", fontsize=9)
...
txt(fig, P[2], 0.07, 0.07, "+ same scores for held-out reference donors\n   (with true labels) for Step 3", fontsize=8, color=MUTED)
save(fig, "step2.png")
